...
global.authFile = `rinoxBotSession`;
...
do {
let lineM = '⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ ⋯ 》'
opcion = await question(chalk.greenBright(`1️⃣ o 2️⃣ :\n1. tramite qr(specifico che si hanno 60sec)\n2. tramite codice (impossibile effettuarlo)\n---> `))
//if (fs.existsSync(`./${authFile}/creds.json`)) {
//console.log(chalk.bold.greenBright(`PRIMERO BORRE EL ARCHIVO ${chalk.bold.greenBright("creds.json")} QUE SE ENCUENTRA EN LA CARPETA ${chalk.bold.blueBright(authFile)} Y REINICIE.`))
//process.exit()
if (!/^[1-2]$/.test(opcion)) {
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['amane-Bot-Md Dev', 'chrome', 'Developer'] : methodCodeQR ? ['rinox-Bot-Md Dev', 'chrome', 'Developer'] : ['Ubuntu', 'Chrome', 'Developer'],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./rinoxBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./amaneBotSession/${files}`)
})
} 
...
const directories = ['./amaneBotSession/', './jadibts/']
...
if (opcion == '1' || methodCodeQR) {
  ...
  if (connection == 'open') {
    console.log(chalk.green('\namane𝐁𝐨𝐭-𝐌𝐝 𝐜𝐨𝐧𝐧𝐞𝐬𝐬𝐨 ✅️ \n'))
  }
let reason = new Boom(lastDisconnect?.error)?.output?.statusCode;
if (reason == 405) {
await fs.unlinkSync("./amaneBotSession/" + "creds.json")
...
setInterval(async () => {
  ...
  const bio = `rinox ⇝MD 𝐨𝐧𝐥𝐢𝐧𝐞 𝐝𝐚 ${uptime} `
